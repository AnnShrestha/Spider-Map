...
CITIES = {
    'kathmandu': {
        ...
        'center': [27.706, 85.315],
        'zoom': 13
    }
}

# Global variables to store the network
current_city = 'kathmandu'  # Track current city
...
def home():
    ...
    # Look for index.html in the parent directory of where this script is located
...
def api_info():
    ...
    cache_file = os.path.join(base_dir, 'CityData', 'ktm_walking_graph.pkl')
    ...
    return jsonify({
        ...
        "endpoints": {
            "/api": "API information",
            "/health": "Health check",
            "/walking-distances-batch": "POST - Calculate walking distances"
        },
        "networkit_loaded": networkit_graph is not None,
        "cache": {
            "exists": cache_exists,
            "size_mb": round(cache_size_mb, 1) if cache_exists else None
        }
    })
...
def list_cities():
    ...
    for key, config in CITIES.items():
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        cache_file = os.path.join(base_dir, 'CityData', f"{config['cache_prefix']}_walking_graph.pkl")
        stations_file = os.path.join(base_dir, 'CityData', f"{config['cache_prefix']}_stations.geojson")
...
def get_stations():
    ...
    try:
        cache_prefix = CITIES.get(current_city, {}).get('cache_prefix', 'ktm')
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        geojson_file = os.path.join(base_dir, 'CityData', f"{cache_prefix}_stations.geojson")
        ...
        if not os.path.exists(geojson_file):
            return jsonify({
                ...
                "city": current_city,
                "count": 0,
                "error": f"No stations file found for {current_city}"
            })
...
def load_networkit_graph():
    ...
    cache_prefix = CITIES.get(current_city, {}).get('cache_prefix', 'ktm')
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    cache_file = os.path.join(base_dir, 'CityData', f"{cache_prefix}_walking_graph.pkl")
    ...
    logger.warning("FALLBACK: Creating new NetworKit graph from OSM data...")
...
@app.route('/walking-distances-batch', methods=['POST', 'OPTIONS'])
def get_walking_distances_batch():
    ...
    try:
        ...
        # Route calculation for specific city if requested
        # Note: Since we only support Kathmandu now, we just ensure it matches
        if requested_city != current_city:
            logger.warning(f"Requested city {requested_city} does not match current city {current_city}")
...
if __name__ == '__main__':
    ...
    defaults = CITIES.get(current_city, {})
    cache_prefix = defaults.get('cache_prefix', 'ktm')
    # CityData directory is expected to be in the parent directory of this script
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    cache_file = os.path.join(base_dir, 'CityData', f"{cache_prefix}_walking_graph.pkl")
    ...
    if os.path.exists(cache_file):
        ...
        logger.warning(f"No cached graph found at {cache_file}")
