...
CITIES = {
    'kathmandu': {
        ...
        'cache_prefix': 'ktm',
        'station_query': {'highway': 'bus_stop'}
    }
}
...
def download_walking_network(city_config):
    ...
    # Ensure CityData directory exists in the root (parent of src)
    ...
    cache_file = os.path.join(city_data_dir, f"{cache_prefix}_walking_graph.pkl")
...
def download_subway_stations(city_config):
    ...
    geojson_file = os.path.join(city_data_dir, f"{cache_prefix}_stations.geojson")
    ...
    try:
        # Download subway stations (or custom query) using OSMnx
        query_tags = city_config.get('station_query', {'station': 'subway'})
        logger.info(f"Using: ox.features_from_place('{city_name}', tags={query_tags})")
        stations_gdf = ox.features_from_place(city_name, tags=query_tags)
...
def main():
    ...
    # Default behavior - download Kathmandu
    logger.info("No city specified, downloading Kathmandu (default)")
    return download_city_data('kathmandu')
